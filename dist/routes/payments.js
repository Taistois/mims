...
router.post("/", authMiddleware_1.verifyToken, (0, authMiddleware_1.authorizeRoles)("admin", "insurance_staff"), async (req, res) => {
    ...
    try {
        ...
        const claimResult = await db_1.default.query(`SELECT c.claim_id, c.member_id, u.user_id, u.name
         FROM claims c
         LEFT JOIN members m ON c.member_id = m.member_id
         LEFT JOIN users u ON m.user_id = u.user_id
         WHERE c.claim_id = $1`, [claim_id]);
        ...
        // 2️⃣ Insert payment
        const paymentResult = await db_1.default.query(`INSERT INTO payments (claim_id, amount, payment_method) 
         VALUES ($1, $2, $3) RETURNING *`, [claim_id, amount, payment_method]);
...
router.delete("/:id", authMiddleware_1.verifyToken, (0, authMiddleware_1.authorizeRoles)("admin"), async (req, res) => {
    const { id } = req.params;
    try {
        const result = await db_1.default.query(`DELETE FROM payments WHERE payment_id = $1 RETURNING *`, [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: "Payment not found ❌" });
        }
