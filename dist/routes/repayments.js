...
router.post("/", authMiddleware_1.verifyToken, (0, authMiddleware_1.authorizeRoles)("admin", "insurance_staff"), async (req, res) => {
    ...
    try {
        ...
        const loanResult = await db_1.default.query(`SELECT l.*, m.user_id, u.name, u.email
         FROM loans l
        ...
        const paymentResult = await db_1.default.query(`INSERT INTO payments (loan_id, amount, method, status) 
         VALUES ($1, $2, $3, 'success') RETURNING *`, [loan_id, amount, method]);
        const repaymentRecord = {
            ...paymentResult.rows[0],
            repayment_id: paymentResult.rows[0].payment_id // map payment_id → repayment_id
        };
        // 3️⃣ Calculate total repaid
        const totalPaidResult = await db_1.default.query("SELECT SUM(amount) AS total_paid FROM payments WHERE loan_id = $1", [loan_id]);
        ...
        if (parseFloat(totalPaid) >= parseFloat(loanAmount)) {
            ...
            await db_1.default.query("UPDATE loans SET status = 'repaid' WHERE loan_id = $1", [loan_id]);
        }
 ...
 */
router.get("/:loan_id", authMiddleware_1.verifyToken, async (req, res) => {
