...
router.post("/", authMiddleware_1.verifyToken, (0, authMiddleware_1.authorizeRoles)("admin", "insurance_staff"), async (req, res) => {
    ...
    try {
        ...
        const result = await db_1.default.query(`INSERT INTO loans (member_id, amount, interest_rate, duration, status, due_date) 
       VALUES ($1, $2, $3, $4, 'pending', $5) RETURNING *`, [member_id, amount, interest_rate, duration, dueDate]);
        const newLoan = result.rows[0];
        // 🔹 Get the user_id for this member
        const memberResult = await db_1.default.query(`SELECT u.user_id, u.name 
       FROM members m 
       JOIN users u ON m.user_id = u.user_id 
       WHERE m.member_id = $1`, [member_id]);
        if (memberResult.rows.length > 0) {
            const user = memberResult.rows[0];
            // 🔹 Create a notification for the member
            await db_1.default.query(`INSERT INTO notifications (user_id, title, message) 
         VALUES ($1, $2, $3)`, [
                user.user_id,
...
router.get("/:id", authMiddleware_1.verifyToken, async (req, res) => {
    try {
        const result = await db_1.default.query("SELECT * FROM loans WHERE loan_id = $1", [
            req.params.id,
        ]);
 ...
 */
router.put("/:id/status", authMiddleware_1.verifyToken, (0, authMiddleware_1.authorizeRoles)("admin", "insurance_staff"), async (req, res) => {
    const { status } = req.body;
    try {
        // 1️⃣ Update loan
        const result = await db_1.default.query(`UPDATE loans SET status = $1 WHERE loan_id = $2 RETURNING *`, [status, req.params.id]);
        if (result.rows.length === 0)
            return res.status(404).json({ error: "Loan not found ❌" });
        const updatedLoan = result.rows[0];
        // 2️⃣ Find loan owner to send notification
        const ownerResult = await db_1.default.query(`SELECT u.user_id, u.name 
       FROM loans l
       JOIN members m ON l.member_id = m.member_id
       JOIN users u ON m.user_id = u.user_id
       WHERE l.loan_id = $1`, [req.params.id]);
        if (ownerResult.rows.length > 0) {
            ...
            if (notificationMessage) {
                await db_1.default.query(`INSERT INTO notifications (user_id, title, message) 
           VALUES ($1, $2, $3)`, [user.user_id, "Loan Status Update", notificationMessage]);
            }
        }
...
router.delete("/:id", authMiddleware_1.verifyToken, (0, authMiddleware_1.authorizeRoles)("admin"), async (req, res) => {
    try {
        const result = await db_1.default.query("DELETE FROM loans WHERE loan_id = $1 RETURNING *", [
            req.params.id,
        ]);
        if (result.rows.length === 0)
            return res.status(404).json({ error: "Loan not found ❌" });
